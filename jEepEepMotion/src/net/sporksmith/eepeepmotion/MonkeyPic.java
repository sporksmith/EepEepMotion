...
public class MonkeyPic {
	  ...
	  float rz = 0.0f; // amt to rotate abt the z axis
	  ...
	  void add_monkey(Monkey m) {
	    ...
	    monkey.rotate(new PVector(0, 0, 1), (float) -this.rz);
//	    monkey.rz -= this.rz;
	    PApplet.println(monkey.toString());
	  }
	  ...
	  void set_scale_max(int maxwidth, int maxheight) {
	    ...
	    float xfactor = (float)maxwidth / this.width;
	    float yfactor = (float)maxheight / this.height; 
	    set_scale(Math.min(xfactor, yfactor), maxwidth/2, maxheight/2);
	    x = 0;
	    y = 0;  
	  }
	  ...
	  void draw() {
	    if (img == null) {
	      img = Globals.applet.loadImage(this.filename);
	      if(this.reflected) {
	        Util.hflip(img);
	      }
	    }
	    ...
	    Globals.applet.translate(rz_x, rz_y, 0);
	    Globals.applet.rotateZ(rz);
	    Globals.applet.translate(-rz_x, -rz_y, 0);
	    
	    Globals.applet.scale((float) scale_factor);
	    Globals.applet.translate(0, 0, -1000);
	    Globals.applet.image(img, -x, -y);
