...
public class MonkeyPics {
    ...
    */
    ...
    try
    {
      ...
      while(in.available() > 0) {
        ...
        PApplet.println("added #" + mps_hm.size() + ": " + loaded.filename);
      }
    }
  ...
  void export() {
    if (this.os != null) {
      try {
        ...
        PApplet.print("couldn't close old outputstream");
      }
      this.os = null;
    }
  ...
  void save(MonkeyPic mp) {
    ...
    if(this.os != null) {
      try {
        ...
        PApplet.print("WARNING couldn't append to index\n");
      }
    } else {
      PApplet.print ("WARNING, no export file set.\n");
    }
    mps_hm.put(mp.filename, mp);
    PApplet.print ("labeled: " + mps_hm.size() + "\n");
  }
  ...
  void set_best_match() {
    this.set_best_match(Globals.applet.frameCount);
  }
  ...
  float match_score(MonkeyPic mp, Monkey m) {
     ...
     int ul_x = Math.max(0, (int)(-mp.x*mp.scale_factor));
     int ul_y = Math.max(0, (int)(-mp.y*mp.scale_factor));
     int lr_x = Math.min(Globals.applet.width, (int)((-mp.x + mp.width)*mp.scale_factor));
     int lr_y = Math.min(Globals.applet.height, (int)((-mp.y + mp.height)*mp.scale_factor));
     float width_in_frame = lr_x - ul_x;
     float height_in_frame = lr_y - ul_y;
     
     frac_frame_used = width_in_frame*height_in_frame / (Globals.applet.width*Globals.applet.height);
     ...
     float rx_diff = PApplet.norm(PVector.angleBetween(mp_monkey.x_axis, m.x_axis), 0, PConstants.PI);
     float ry_diff = PApplet.norm(PVector.angleBetween(mp_monkey.y_axis, m.y_axis), 0, PConstants.PI);
     float rz_diff = PApplet.norm(PVector.angleBetween(mp_monkey.z_axis, m.z_axis), 0, PConstants.PI);
     ...
     score = rx_diff + ry_diff + rz_diff + 0.1f*(1.0f - frac_in_frame) + 1*(1.0f - frac_frame_used) + 0.5f*scale_penalty; //+ abs(Globals.mp.rz);     
     ...
     PApplet.println(mp.filename);
     PApplet.println ("scoring:" 
            + " rx:" + rx_diff 
            + " ry:" + ry_diff 
            + " rz:" + rz_diff 
  ...
  MonkeyPic best_match(Monkey m) {
    return this.best_match(m, Globals.applet.frameCount);
  }
