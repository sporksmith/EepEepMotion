...
class KeyFrameKeeper {
  ...
  void put(int frame, String filename) {
    ...
    //clunky workaround...
    int sched_frame = Math.max(2, frame-this.dq_frames);
  ...
  void disqualify_pic(String filename, int until_frame) {
      ...
      if(mp == null) {
        PApplet.println("cant find file to dq: " + filename);
      }
...
class RenderFromFramesMode extends Mode{
  ...
  void draw() { // ugh. have to return from draw to actually draw anything. makes following code to preview while rendering a bit awkward.
    ...
    Globals.applet.background(0xffffffff);
    Globals.m.move_to();
    Globals.m.draw_color(0xff000000, 0xff000000);
    Globals.applet.saveFrame(mask_file.getPath());
    Globals.applet.popMatrix();
    
    
    // create and save green/white mask based on monkey
    Globals.applet.pushMatrix();
    Globals.applet.background(0xff00ff00);
    Globals.m.move_to();
    Globals.m.draw_color(0xff000000, 0xff000000);
    Globals.applet.saveFrame(imask_file.getPath());
    Globals.applet.popMatrix();
    ...
    Globals.applet.saveFrame(out_file.getPath());
...
//      key_frames.put(1709, "Lawton, Oklahoma/Code Monkey32 Cake Lawton Ok.JPG"); // nap
  ...
  void readFrames() {
    ...
    String lines[] = PApplet.loadStrings(framesfile);
    for (int i=0; i < lines.length; i++) {
      ...
      frame_num = Integer.parseInt(st.nextToken());
      x = Float.parseFloat(st.nextToken()) * 2 + 320;// * width / 640;
      y = Float.parseFloat(st.nextToken()) * 2 + 240;// * height / 480;
      z = Float.parseFloat(st.nextToken());
      rx = Float.parseFloat(st.nextToken());
      ry = Float.parseFloat(st.nextToken());
      rz = Float.parseFloat(st.nextToken());
      scale_factor = Float.parseFloat(st.nextToken()) * 2;// / 36.571;
...
//      m.rotate(new PVector(1, 0, 0), rx);
      ...
      PApplet.println("" + frame_num + " " + m.x + " " +  m.y);
     /* 
