LinkedList reversedLinkedList(LinkedList ll) {
...
PVector rotatePointAroundVector(PVector m, PVector n, float a) {
  float x = m.x;
  float y = m.y;
  float z = m.z;
  float u = n.x;
...
float normalize_radians(float r) {
  Boolean flip_sign;
  if (r < 0) {
    r = -r;
    flip_sign = true;
...
float angle_of(float x, float y) {
